...
<link rel="stylesheet" type="text/css"
	href="/resources/include/css/commons.css">
<link rel="stylesheet" type="text/css"
	...
	function boardPwdConfirm() {
		if (!chkSubmit($('#b_pwd'), "비밀번호를 ")) {
			...
			$.ajax({
				...
				dataType : "text",
				error : function() {//실행시 오류발생시 
					alert('시스템 오류 입니다. 관리자에게 문의 하세요');
				},
				success : function(resultData) {
					var goUrl = ""; //이동할 경로를 저장할 변수
					if (resultData == "실패") {//일치하지 않는경우
						$("#msg").text("작성시 입력한 비밀번호가 일치하지않습니다.").css("color",
								"red");
						$("#b_pwd").select();
				...
				<td id="btd1"><div id="pwdChk">
						<form name="f_pwd" id="f_pwd">
							<input type="hidden" name="b_num" id="b_num"
								value="${detail.b_num}" />
							<lable for="b_pwd" id="l_pwd"> 비밀번호 : </lable>
							<input type="password" name="b_pwd" id="b_pwd" id="b_pwd" /> <span
								id="msg"></span>
						</form>
					</div></td>
